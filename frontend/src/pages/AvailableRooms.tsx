...
const RoomSelection = () => {
  ...
  const promoPercent = location.state?.promo?.discount;
    ...
    const fetchRooms = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `https://backend.bahamaslrb.com?checkInDate=${checkin}&checkOutDate=${checkout}&promoCode=${promo}`,
          {
            headers: {
    ...
    return (
      <>
        ...
        <section className="bg-white">
          ...
          <div className="py-12 sm:py-16 bg-gradient-to-b">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12">
      ...
      <section className="bg-white">
        ...
        <div className="py-12 sm:py-16 bg-gradient-to-b">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-12">
            <h2 className="text-2xl font-bold mb-4 text-black text-center mt-10">
              Rooms available
            </h2>
            <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
              {/* Check-in Date */}
              <div className="flex flex-col items-center justify-center bg-white border border-gray-200 shadow-[0_0px_3px_rgba(255,122,0,0.2)] rounded-lg p-4">
                <p className="text-sm text-gray-500 mb-1">Check-in Date</p>
                <span className="text-lg font-semibold text-[#F9862D]">
                  {checkin}
                </span>
              </div>
            ...
            {error && (
              <div className="flex flex-col items-center justify-center text-center py-10 px-6">
                <img
                  ...
                  className="w-48 h-48 sm:w-56 sm:h-56 mb-6"
                />
                <h2 className="text-xl sm:text-2xl font-semibold text-gray-800 mb-2">
                  No Rooms Available
                </h2>
                <p className="text-gray-600 text-sm sm:text-base mb-4 max-w-md">
                  Unfortunately, we couldn't find any available rooms matching
                  your selected check-in date, check-out date, or promo code.
                </p>
                <Link to="/">
                  <button className="bg-[#F9862D] hover:bg-[#e67b23] text-white px-6 py-2 rounded-md text-sm font-medium shadow-sm hover:shadow-md transition duration-200 cursor-pointer">
                    Back to Home
                  </button>
                </Link>
              </div>
                ...
                >
                  <div className="relative">
                    ...
                    <div className="absolute top-2 sm:top-3 left-2 sm:left-3 z-20">
                      <Badge className="bg-gradient-to-r bg-white text-[10px] sm:text-xs px-2 sm:px-3 py-0.5 sm:py-1 rounded-md shadow-md flex items-center gap-1 sm:gap-1.5">
                        {promoPercent ? (
                          <span className="line-through text-gray-400 mr-1">
                            {room?.price.toLocaleString("En-US")} Fr
                          </span>
                        ) : (

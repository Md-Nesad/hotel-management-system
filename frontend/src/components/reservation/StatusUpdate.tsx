...
const ReservationStatus = ({ bookingId }: { bookingId: string }) => {
    ...
    <div>
      <div className="relative w-fit">
        <select
          ...
          className={`border border-gray-300 rounded-md py-2 pl-1 pr-5 appearance-none ${
            status === "check in"
        ...
        <FiChevronDown className="absolute right-2 top-3 text-white pointer-events-none" />
      </div>
    </div>
  );
};

...
export default function FoodListImageOne() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        "https://backend.bahamaslrb.com/api/food/create/block1", // replace with actual API
        formData,
        {
          headers: {
        ...
        <div>
          <div className="w-[280px] h-40 mx-auto bg-gray-300 rounded-md relative overflow-hidden cursor-pointer">
            {image1 ? (
              ...
              <img
                src={
                  foodData && foodData.img1
                    ? `https://backend.bahamaslrb.com/uploads/${foodData.img1}`
                    : "no image found"
                }
                alt="First food image"
              />
            )}
            <input

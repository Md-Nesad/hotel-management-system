...
export default function HeroImageTwo() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (image1) formData.append("img2", image1);
      formData.append("img2Title", title1);
      formData.append("button2", button1);

      const response = await axios.post(
        "https://backend.bahamaslrb.com/api/page/create-image2",
        formData,
        {
          headers: {
  ...
  return (
    <>
      ...
      >
        <h2 className="text-center text-xl font-bold mb-5">Hero Image 2</h2>
        <div>
          <div className="w-[280px] mx-auto bg-gray-300 rounded-md relative overflow-hidden cursor-pointer">
            {image1 ? (
              ...
              <img
                src={
                  pageData && pageData.img2
                    ? `https://backend.bahamaslrb.com/uploads/${pageData.img2}`
                    : "no image found"
                }
                alt="home page two"
              />
          ...
          <p className="text-center text-sm text-gray-500 mt-2 mb-7">Image 2</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full border-b border-gray-200 pb-8">
          <div>
            <label className="text-sm font-bold">Image 2 Title</label>
            <input
              type="text"
          ...
          <div>
            <label className="text-sm font-bold">Button 2</label>
            <input
              type="text"

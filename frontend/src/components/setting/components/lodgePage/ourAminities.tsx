...
export default function OurAminities() {
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      amenitiesSubTitle: subTitle,
    };

    try {
      const res = await fetch(
        "https://backend.bahamaslrb.com/api/page/create-amenities",
        {
          method: "POST",
          headers: {
      ...
      alert("Amenities section saved successfully!");

      setTitle("");
      setSubTitle("");
    } catch (error) {
      console.error("Error:", error);
      alert("Failed to save amenities section.");
    } finally {
  ...
  return (
    <>
      ...
      <section className="px-8 py-4 w-full mx-auto bg-white rounded-b-xl shadow-md">
        <h2 className="text-center text-xl font-bold mb-10">OUR AMENITIES</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input
              ...
              placeholder="Enter Aminities Title"
          ...
          <div>
            ...
            <input
              ...
              placeholder="Enter Aminities Sub Title"
              className="w-full py-2 pl-4 border border-gray-300 rounded bg-gray-300 mt-2"
            />
          </div>
        </div>

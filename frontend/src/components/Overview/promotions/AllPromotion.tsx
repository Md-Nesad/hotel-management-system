...
const AllPromotionList = ({
  ...
  const handleShare = async (promo: Promotion) => {
    const shareText = `🎉 Use promo code: ${promo.promoCode} to get ${
      promo.discount
    }% off! Valid till ${new Date(promo.endDate).toLocaleDateString()}`;
    ...
    try {
      if (navigator.share) {
        ...
        await navigator.share({
          title: "Promo Code",
          text: shareText,
          url: window.location.origin, // তোমার website url
        });
      } else {
...
export default AllPromotionList;

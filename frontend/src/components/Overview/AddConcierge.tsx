...
export default function AddConcierge() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const data: any = {
      ...
      scheduleDay,
      scheduleTime,
      assignedFloor: Number(assignedFloor),
      salary,
      dutyStatus,
      specialNote,
      address,
    ...
    try {
      ...
      const response = await fetch(
        "https://backend.bahamaslrb.com/api/concierges/create",
        {
          method: "POST",
          headers: {
      ...
      console.log(result);
      alert(result?.message || "Add concierge successfully");
      setAllEmployeeIds([...allEmployeeIds, employeeId]); // ✅ Update local cache
      resetForm();
    } catch (error) {
